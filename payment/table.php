...
<?php include("../float.php");?>
    <div class="container mt-5">
        <div class="row">
            <div class="d-flex justify-content-between">
                ...
                <a href="index.php" class="btn btn-primary"><i class="bi bi-plus-circle"></i> New Payment</a>
            </div>
  ...
  <script>
     fetch("http://localhost/erp/payment/read.php").then(
            res => {
                res.json().then(
                    data => {
                        if(data.data.length > 0){
                            ...
                            data.data.forEach((itemData)=>{
                                ...
                                temp +="<td><a href='../purchase/table.php' style='text-decoration:none;'>" + itemData.bill_no+"</a></td>";
        ...
        $(document).on('click','#delete',function(){
                ...
                var paid=$(this).closest('tr').find("td:eq(4)").html();
                var type=$(this).closest('tr').find("td:eq(6)").html();
                var confirmation=confirm('Are you delete this item?');
                if(confirmation){
                    var deleteURL="http://localhost/erp/payment/delete.php?id="+id+'&paid='+paid+'&type='+type;
                    $.ajax({
                        url:deleteURL,
                        method:'DELETE',
                        success:function(){
            ...
            paid: $(this).closest('tr').find('td:eq(4)').text(),
        ...
        if(confirmation){
            window.location.href="http://localhost/erp/payment/form_update.php?id="+ id +
            ...
            "&paid=" + encodeURIComponent(lastClickedCustomer.paid)+
