...
<?php include("../float.php");?>
    <div class="container mt-5">
        <div class="row">
            <div class="d-flex justify-content-between">
                <h5><i class="bi bi-journal-richtext"></i> Income Details</h5>
                <a href="index.php" class="btn btn-primary"><i class="bi bi-plus-circle"></i> New Income</a>
            </div>
  ...
  <script>
     fetch("http://localhost/erp/income/read.php").then(
            res => {
                res.json().then(
                    data => {
        ...
        $(document).on('click','#delete',function(){
                ...
                var amnt=$(this).closest('tr').find("td:eq(3)").html();
                var type=$(this).closest('tr').find("td:eq(4)").html();
                var confirmation=confirm('Are you delete this item?');
                if(confirmation){
                    var deleteURL="http://localhost/erp/income/delete.php?id="+id+'&amnt='+amnt+'&type='+type;
                    $.ajax({
                        url:deleteURL,
                        method:'DELETE',
                        success:function(){
            ...
            payment: $(this).closest('tr').find('td:eq(4)').text(),
        };
        var confirmation=confirm ('Are you edit this data?');
        if(confirmation){
            window.location.href="http://localhost/erp/income/form_update.php?id="+ id +
